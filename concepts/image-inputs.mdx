...
one that governs every page here: `POST /v1/chat/completions` and
`GET /v1/models` are the whole public surface, so there is no image generation
endpoint, no speech endpoint, and no video endpoint to describe. See
