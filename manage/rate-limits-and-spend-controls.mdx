...
status: partial
gates: ["10.11", "9.6", "9.26", "21.48", "22.4"]
---
...
| 429 | `spend_rate_exceeded` | Your spend rate cap, which counts money reserved rather than money spent. | Wait `Retry-After` seconds, up to 60, or send `max_tokens`. See below. |
...
| 402 | `workspace_limit_exceeded` | The workspace's monthly spend limit, for the current calendar month. | Ask the workspace owner, whose control for it is not built into the dashboard yet, or ask us. |
...
<FeatureStatus>
  One further 429 meaning is specified and not yet shipped, for a provider pool we are
  protecting. It is deliberately absent from the table above rather than described
  early. The other one this callout used to name, a provider refusing us, has shipped:
  it is the `upstream_rate_limited` row above. Until the remaining one ships, treat a
  429 carrying none of the codes above as a condition to retry after a short pause,
  and quote the request id if it persists.
</FeatureStatus>
...
The refusal carries `Retry-After` in whole seconds, computed from that same earliest
reservation, so the header and the sentence in the message name the same wait. A
client that backs off on `Retry-After` needs no special case for this code.
...
key](/features/bring-your-own-key).
...
The spend rate cap is a whole number of at least 1 minor unit per rolling 60
