...
| Served by | The provider that answered, or **nothing** if none did. A **backup** mark beside it means this request came back through a backup route; the provider named is still the one you asked for. |
...
| Charge | What the request cost, as dollars to as many places as the figure needs and never fewer than two, or **unpriced** on an older row that carries no charge figure at all. |
| Took | How long the request took, in milliseconds. |
| Request id | The first eight characters, linking to the single request view. |

## Reading a row

**A `backup` mark beside the provider.** It appears on a row, on that row's expanded
facts and on the single request view, and it says one thing: this answer came back
through a backup route rather than the ordinary one. The model you asked for is still
the model that answered, and the same fact reaches an API caller as the
`x-hopscotch-served-via` response header, described under
...
**`unpriced` is not free, and no new row reads it.** A row with no charge figure at
all reads `unpriced`; a charge of `0` is printed as money, and only a request that was
actually priced at zero shows a zero. Since 2026-09-10 the platform does not write a
row without a charge figure: a request that was served is always charged, from the
amount you accepted when it was admitted where nothing better can be worked out, and a
request that was refused carries a real `0` rather than a blank. So `unpriced` now
describes rows recorded before that change, and the column keeps the word so an older
row still reads honestly rather than turning into a zero it never was.
...
following the same rules above: a refusal you were not charged for shows a real `0`,
which is the fact that nothing was taken, and never a blank.
...
took, and the outcome. See [Data and privacy](/manage/data-and-privacy) for the
...
<CardGroup cols={2}>
  ...
  <Card title="Data and privacy" href="/manage/data-and-privacy">
