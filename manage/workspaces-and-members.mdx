...
description: "A workspace holds your keys, usage, playground and billing. Roles decide who may do what: the owner and admins buy credit, and the owner alone pauses."
status: partial
...
| Buy credit and manage the saved cards | Yes | Yes | No | No |
| Pause or resume requests, and set the low credit notice | Yes | No | No | No |
...
Money reads are the workspace's. Buying credit and managing the card belong to the
owner and admins; pausing belongs to the owner alone.
...
Buying credit and managing the saved cards are the owner's and every admin's.
Every billing control answers a developer or a viewer with one sentence:

> Only the workspace owner or an admin can manage its billing: its saved cards and
> the buying of credit. Every member may read the balance and the statement of the
> workspace they are in.

Pausing or resuming requests and changing the low credit notice are the owner's
alone, and have their own sentence, because the fact they refuse is not billing:

> Only the workspace owner can pause or resume its requests, or change when it is
> emailed about low credit.
...
- It may carry a key reservation, so a person arrives with a key waiting for
  ...
  moment it is made. See [Create an API key](/get-started/create-an-api-key).
...
<CardGroup cols={2}>
  <Card title="Create a workspace" href="/manage/create-a-workspace">
    Open one from the workspace switcher, and see what you get on day one.
  </Card>
  <Card title="Invite and manage members" href="/manage/invite-and-manage-members">
