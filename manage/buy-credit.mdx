...
You need to be the **owner** or an **admin** of the workspace. Developers and viewers
can read the balance and the statement, and cannot buy. One who calls a purchase
route gets HTTP 403 carrying
the sentence above, so nobody starts a purchase that was never going to complete.

You also need the smallest purchase we accept, which is **5.00 of credit**. That is
the credit you receive; our fee, currently 0%, is added on top at the checkout.
...
fee, as three sub-fields of the one line, which is what explains any gap between your
...
The Statement tab is live today, one of the two tabs on the Billing screen alongside
Balance. What you see there is read from your ledger, not a preview of a screen still
being designed.
...
Your card is kept by our payment provider so that the next purchase does not ask for
it again. We store no card number and no expiry: the brand and the last four digits
are read from the provider when they are shown, so there is nothing on our side that
can go stale. A saved card can be removed, which means the next purchase asks for a
card again; it changes nothing about credit you have already bought and nothing about
your balance.

<Note>
  **The Billing screen has no card controls at the moment.** The tab that held them
  was taken off Billing on 2026-09-11 while the controls are being worked on, so
  Billing is Balance and Statement. Buying credit is untouched and is still on the
  Balance tab, reading the same terms, and a card saved during a purchase is still
  kept and still used for the next one. If you need a card removed while the controls
  are off the screen, ask support.
</Note>

There is no automatic top up: nothing charges your card while you are away, and there
is no setting that turns that on.
...
<CardGroup cols={2}>
  ...
  <Card title="Spend controls" href="/manage/spend-controls">
