---
title: "Glossary"
description: "The words this site uses for Hopscotch's own concepts, defined once and used the same way everywhere."
status: partial
gates: ["18.13", "18.24", "18.40", "18.62"]
---
...
[Create an API key](/get-started/create-an-api-key).
...
gate a request. See [Model capabilities](/features/model-capabilities).

### Credit

Prepaid spending power on a workspace, bought through checkout and reduced by
usage at face value. Credit is bought at face value plus the platform fee, which is
currently 0%. A purchase therefore has two figures: the amount charged to the card,
and the credit granted. On this site, "credit" always means the second.

### Deprecation

The date a provider stopped recommending a model. The model still answers. It is
not the same as a sunset. See [Model lifecycle](/features/model-lifecycle).
...
[Invite and manage members](/manage/invite-and-manage-members).
...
The one member of a workspace who can pause its requests and set its spend
limits, and who shares the card and the buying of credit with its admins. A workspace has exactly one owner, always.
...
purchase and never taken when a request is served. Currently 0%. The rate that
...
no prompt or reply. See [Playground](/features/playground).
...
reports them at all. See [Reasoning models](/features/reasoning).
...
rank. See [Workspaces and members](/manage/workspaces-and-members).
...
only the owner or an admin may buy credit.
...
than a shutdown. See [Model lifecycle](/features/model-lifecycle).
...
and never the reply. See [Usage and metering](/manage/usage-and-metering).
