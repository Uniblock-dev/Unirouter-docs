...
A bare id such as `gpt-4o-mini` is not a Uniblock model id. Ids name the provider
...
`POST /v1/chat/completions` and `GET /v1/models`, plus `GET /v1/models/{id}`.
That is the whole public surface. Any other path under `/v1` answers `404` with
the code `unknown_url`. See
...
Not yet. It answers `unknown_url` today, and it is not documented anywhere on
this site until it serves real responses.
</Accordion>
