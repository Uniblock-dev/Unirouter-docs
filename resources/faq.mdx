...
[Migrate from OpenAI](/get-started/migrate-from-openai).
</Accordion>
...
we happen to carry it under. See [Models](/features/models).
</Accordion>
...
reply against it. See [Structured outputs](/features/structured-outputs).
</Accordion>

<Accordion title="Can I send images?">
Yes, in OpenAI's message-part shape, to a model that takes one. See
[Image inputs](/features/image-inputs).
</Accordion>
...
See [Model capabilities](/features/model-capabilities).
</Accordion>
...
to enable. See [Prompt caching](/features/prompt-caching).
</Accordion>
...
[Routing profiles](/features/routing-profiles).
</Accordion>
...
`spend_rate_exceeded` is your account spend rate cap, which also carries
`Retry-After`, and the lever for it is
...
[View activity](/manage/view-activity).
</Accordion>
...
catalog. See [Bring your own key](/features/bring-your-own-key).
</Accordion>
