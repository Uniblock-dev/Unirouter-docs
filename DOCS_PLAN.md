...
13. **The public base URL every page prints is not the host the gateway now serves.** This plan, the PRD's FR-1 and the whitepaper all give `https://ai.uniblock.dev`, and §8 rewrites the artifact's `servers` to exactly that. Two facts moved against it. The tracker's Story 2.3 note says `ai.uniblock.dev` is still NXDOMAIN and that provisioning the zone needs DNS administration the account does not hold, so 2.3 stays `in-progress`. And the source now attaches Custom Domains of its own: `dev.unirouter.app` on dev, answering already, and `api.unirouter.app` on production, which the change says attaches on the next main deploy. So the host the docs print does not resolve, and the host that will resolve is not the one the PRD names. This is an owner decision and not a writer's: it is a customer-facing brand name carried by FR-1, the whitepaper, every code example on the site and `PRODUCTION_SERVER` in `scripts/lib/paths.mjs`. Nothing was rewritten in this pass. Unblock: the owner names the customer base URL, and it answers. Recorded in the internal site's unresolved findings.
