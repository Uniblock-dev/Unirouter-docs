...
and `ANTHROPIC_API_KEY` sends `x-api-key`. Every path this site publishes reads
`Authorization: Bearer`, so `ANTHROPIC_AUTH_TOKEN` is the setting that matches
the rest of this API, and it is also the one Anthropic recommends for a
third-party gateway. [Authentication](/get-started/authentication) covers the header.
