---
title: "Zed"
description: "Configure Zed's openai_compatible provider against Uniblock: api_url, the models you declare yourself, and the capability that must stay on."
status: partial
gates: ["2.3", "2.11"]
---
...
```json settings.json
{
  "language_models": {
    "openai_compatible": {
      "uniblock": {
        "api_url": "https://ai.uniblock.dev/v1",
        "available_models": [
          {
            "name": "openai/gpt-4o-mini",
            "display_name": "GPT-4o mini via Uniblock",
            "max_tokens": 128000
          }
        ]
      }
...
which endpoint Zed calls. `chat_completions` defaults to `true`, which is what
this API serves. Setting it to `false` tells Zed to use the Responses endpoint
instead, which is not part of this API's published surface, so every request from
that model would answer `404` with `unknown_url`.
