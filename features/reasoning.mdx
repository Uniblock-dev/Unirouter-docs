...
take comes back as the provider's own refusal rather than as one of ours, with
one exception. An empty or whitespace-only `reasoning_effort` is the one value no
provider accepts, and it is refused here with a `400`, `code` `invalid_value` and
`param` set to `reasoning_effort`, rather than sent on to be refused a second
later. Everything else, including the booleans and integers some providers take,
travels as you wrote it. See [Request parameters](/api-reference/parameters).
...
See [Models](/features/models) for the whole vocabulary, and read what a request
...
final counts to report. See [Streaming](/features/streaming).
