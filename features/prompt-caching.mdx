...
**On the stream**, as fields on the `:x-hopscotch-usage` comment line written
...
| `cache_write_1h_input_tokens` | The part of those writes made with a one-hour lifetime. Already counted inside `cache_write_input_tokens`, and present only when the provider split its writes by lifetime, as Anthropic does. |
...
Each field is omitted rather than sent as `0` when the provider did not report
it. A provider that does no cache accounting produces a usage line with neither
...
[Models](/features/models) for the whole rate vocabulary.
