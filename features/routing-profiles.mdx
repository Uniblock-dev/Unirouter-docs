...
[Models](/features/models) for how a plain model id is shaped.
...
identifier. See [Errors and refusals](/features/errors) for the envelope every
...
`hopscotch/auto` is accepted as a second spelling of the same id and resolves
identically, so a configuration written either way keeps working and there is
nothing to migrate.
...
you have [brought your own key](/features/bring-your-own-key) for a provider we
...
already can. See [Workspaces and members](/manage/workspaces-and-members) for
the full role table.
...
<CardGroup cols={2}>
  ...
  <Card title="Errors and refusals" href="/features/errors">
