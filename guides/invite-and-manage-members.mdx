...
Members are managed in the dashboard, at [Members](https://app.uniblock.dev).
This page describes the rules each flow enforces. For the roles themselves and
what each one may do, read
[Workspaces and members](/concepts/workspaces-and-members).

<Note>
The member flows below are described from the rules the service enforces, not
from a screen we walked. Several of them have not been accepted yet: the roster,
the role editor, ownership transfer and the removal flow are all still open work.
Where a screen and this page disagree, the screen is what you are using.
</Note>
...
**A second workspace.** Opening another workspace and switching between them
have not been accepted, so they are not documented.
