...
| Outcome | The raw outcome slug: `served`, or the reason it was not. |
| Tokens | The total token count, or **unknown** if none was ever reported. |
| Charge | The cost in minor units, or **unpriced** if the request was never priced. |
| Took | How long the request took, in milliseconds. |
| Request id | The first eight characters, linking to the single request view. |

## Reading a row

**Outcome is the raw word, not a translation.** A row whose outcome is anything
other than `served` is drawn in a distinct color to draw the eye. The word itself is
whatever the platform recorded, never a friendlier rewrite of it.
...
|---|---|---|
| Model | Only the models this workspace has actually called. | Any model |
| Outcome | Free text, matched against the outcome the platform recorded. | Empty (no default vocabulary is baked into the field) |
...
There is no rows-per-page control. Page size is fixed at fifty and reported by the
platform rather than accepted as something you set. Rows are ordered by time with
the request id as a tiebreak, so a row cannot land on two pages, or on none, between
