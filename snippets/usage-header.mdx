...
| `cache_write_1h_input_tokens` | integer | The part of `cache_write_input_tokens` written with a one-hour lifetime. Part of that figure, never added to it. Omitted when the provider did not split its cache writes by lifetime. |
