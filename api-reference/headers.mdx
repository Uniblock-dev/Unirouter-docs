...
| `x-uniblock-served-via` | Only when a backup route served the request | The single value `backup`. Absent on every other response. |

**Every response header above that begins `x-uniblock-` is also sent under
`x-hopscotch-`**, with the same value, on the same responses. The two spellings
are the same header: read whichever you prefer, and read only one. The
`x-uniblock-` spelling is not going away, so nothing you already parse has to
change.
