---
title: "API reference"
description: "The Hopscotch API is OpenAI-shaped: one base URL, one bearer key, and the request and response bodies your client already sends."
status: write-now
gates: []
---
...
names the provider that serves the request, and everything after it is our name for that model on
that provider: the provider's own id with the maker's namespace taken off, so a model a provider
publishes as `openai/gpt-oss-120b` is called `groq/gpt-oss-120b` here. We send the provider its own
spelling on your behalf. A bare id with no provider is refused with `400` and the code
`model_id_not_pinned`. Call `GET /v1/models` for the exact strings you can use, and see
[Models](/features/models) for the whole rule.
...
[Streaming](/features/streaming) for the full transcript, including the one
