...
status: write-now
gates: [2.9, 9.6, 9.26, 13.9, 15.8, 21.48, 22.4, 36.17]
---
...
| 400 | `invalid_request_error` | `null` | On the platform's own provider keys, a field that buys something this platform cannot bill yet or asks the provider to keep the conversation, such as a non-standard `service_tier`, a hosted tool the provider charges for per call, audio, a one-hour cache write, or `store: true`. `param` names the field and never its value, and the message says to remove it or send the request on your own provider key, where every one of them is forwarded. Refused before admission, so nothing is held and no provider is called. The list is on [Request parameters](/api-reference/parameters). |
...
| 429 | `rate_limit_error` | `spend_rate_exceeded` | The account's spend rate cap. Nothing to fix; requests are admitted again as the window rolls forward. Carries `Retry-After` in whole seconds: the wait until the window has no counted reservation left, computed from the earliest reservation still counted, and never less than one second. |
...
- `spend_rate_exceeded` is the account's spend rate cap, which counts the
  ...
  It carries `Retry-After` in whole seconds, computed from the earliest
  reservation still counted in the window, so the header and the wait named in
  the message cannot disagree. Sending `max_tokens` on your requests lowers the
...
`Retry-After` is present on all three `429` codes and is not sent on other
statuses. Do not infer a retry delay from its absence. It was **not** sent on
`spend_rate_exceeded` until 2026-09-16, when the wait moved out of the English
message and into a header; a client written against the older behaviour, which
read the sentence or backed off on a fixed timer, keeps working unchanged.
