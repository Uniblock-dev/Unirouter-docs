...
| 404 | `not_found_error` | `model_not_available` | The id is catalogued and is not being served right now, either at all or on the endpoint you called. Retrying later is worth more than changing the request. Told apart from `model_not_found` so a withdrawn model does not read as a typo. |
